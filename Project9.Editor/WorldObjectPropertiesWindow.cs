...
{
    /// <summary>
    /// Dockable window for editing world object (furniture) properties
    /// </summary>
    public class WorldObjectPropertiesWindow : Form
    {
        ...
        public WorldObject? CurrentWorldObject
        {
            ...
            set
            {
                _currentWorldObject = value;
                UpdatePropertyGrid();
            }
        }

        public WorldObjectPropertiesWindow()
        {
            InitializeComponent();
        }
        ...
        {
            this.Text = "World Object Properties";
            ...
            {
                if (_currentWorldObject != null)
                {
                    using (ColorDialog colorDialog = new ColorDialog())
                    {
                        // Set current color
                        colorDialog.Color = Color.FromArgb(
                            _currentWorldObject.BoundingBoxColorR,
                            _currentWorldObject.BoundingBoxColorG,
                            _currentWorldObject.BoundingBoxColorB
                        );
                        ...
                        if (colorDialog.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            _currentWorldObject.BoundingBoxColorR = colorDialog.Color.R;
                            _currentWorldObject.BoundingBoxColorG = colorDialog.Color.G;
                            _currentWorldObject.BoundingBoxColorB = colorDialog.Color.B;
                            
                            // Update button color
                            _colorPickerButton.BackColor = colorDialog.Color;
                            
                            // Refresh property grid and preview
                            UpdatePropertyGrid();
        ...
        private void UpdatePropertyGrid()
        {
            if (_currentWorldObject != null)
            {
                ...
                _nameTextBox.TextChanged -= _nameTextBox_TextChanged;
                _nameTextBox.Text = _currentWorldObject.Name ?? "";
                _nameTextBox.TextChanged += _nameTextBox_TextChanged;
                
                // Create a wrapper object for property grid editing
                var wrapper = new WorldObjectPropertiesWrapper(_currentWorldObject);
            ...
            else
            {
                ...
                _titleLabel.Text = "  World Object Properties  •  No Selection";
            }
        ...
        private void _nameTextBox_TextChanged(object? sender, EventArgs e)
        {
            if (_currentWorldObject != null)
            {
                _currentWorldObject.Name = _nameTextBox.Text;
                _onSaveCallback?.Invoke(); // Auto-save on change
                UpdateTitle();
            }
        }
        ...
        private void UpdateTitle()
        {
            if (_currentWorldObject != null)
            {
                string nameDisplay = string.IsNullOrWhiteSpace(_currentWorldObject.Name) ? "Unnamed" : _currentWorldObject.Name;
                _titleLabel.Text = $"  World Object Properties  •  {nameDisplay}  •  X: {_currentWorldObject.X:F1}, Y: {_currentWorldObject.Y:F1}";
            }
        }
        ...
        private void PropertyGrid_PropertyValueChanged(object? s, PropertyValueChangedEventArgs e)
        {
            ...
            if (_currentWorldObject != null && (e.ChangedItem?.Label == "Name" || e.ChangedItem?.Label == "X" || e.ChangedItem?.Label == "Y"))
            {
                UpdateTitle();
            }
        ...
        /// <summary>
        /// Wrapper class to expose WorldObject properties for PropertyGrid editing
        /// </summary>
        private class WorldObjectPropertiesWrapper
        {
            private WorldObject _worldObject;

            public WorldObjectPropertiesWrapper(WorldObject worldObject)
            {
            ...
            {
                get => _worldObject.DiamondWidth;
                set
                {
                    _worldObject.DiamondWidth = value;
                    // Maintain isometric 2:1 aspect ratio (width:height)
                    _worldObject.DiamondHeight = value / 2;
                }
            }
            ...
            {
                get => _worldObject.DiamondHeight;
                set
                {
                    _worldObject.DiamondHeight = value;
                    // Maintain isometric 2:1 aspect ratio (width:height)
                    _worldObject.DiamondWidth = value * 2;
                }
            }
            
            [System.ComponentModel.Category("Visual")]
            [System.ComponentModel.Description("Z height for 3D isometric rendering (vertical offset, in divisions of 32)")]
            public float ZHeight
            {
                get => _worldObject.ZHeight;
                set => _worldObject.ZHeight = Math.Max(0.0f, value);
            }
            ...
            {
                get => _worldObject.BoundingBoxColorR;
                set => _worldObject.BoundingBoxColorR = value;
            }
            ...
            {
                get => _worldObject.BoundingBoxColorG;
                set => _worldObject.BoundingBoxColorG = value;
            }
            ...
            {
                get => _worldObject.BoundingBoxColorB;
                set => _worldObject.BoundingBoxColorB = value;
            }
            ...
            {
                get => _worldObject.BoundingBoxOpacity;
                set => _worldObject.BoundingBoxOpacity = Math.Clamp(value, 0.0f, 1.0f);
            }
        }
        
        private void DrawBoundingBoxPreview(Graphics g, Rectangle bounds)
        {
            if (_currentWorldObject == null) return;
            ...
            float width = _currentWorldObject.DiamondWidth;
            float height = _currentWorldObject.DiamondHeight;
            float zHeight = _currentWorldObject.ZHeight;
            ...
            if (zHeight > 0)
            {
                ...
                float previewOpacity = _currentWorldObject.BoundingBoxOpacity;
            ...
            else
            {
                ...
                // Draw filled diamond with entity's opacity
                float previewOpacity = _currentWorldObject.BoundingBoxOpacity;
                int alpha = (int)(previewOpacity * 255.0f);
                Color fillColor = Color.FromArgb(alpha, boxColor);
                using (SolidBrush fillBrush = new SolidBrush(fillColor))
